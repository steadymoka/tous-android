...
public class PagerSlidingTabStrip extends HorizontalScrollView {
	...
	private int selectedTabTextColor = 0xFFED1844;
	private Typeface tabTypeface = null;
	private int tabTypefaceStyle = Typeface.BOLD;

	private int lastScrollX = 0;

	private int tabBackgroundResId = R.drawable.selector_background_sliding_tab;

	private Locale locale;

	private ArrayList<View> imageButtons = new ArrayList<View>();
	private View selectedView = null;

	private OnScrollingToTopListener onScrollingToTopListener;
	...
	public void setOnScrollingToTopListener( OnScrollingToTopListener onScrollingToTopListener ) {

		this.onScrollingToTopListener = onScrollingToTopListener;
	}
	...
	public void notifyDataSetChanged() {
		...
		for ( int i = 0; i < tabCount; i++ ) {
			...
			if ( pager.getAdapter() instanceof IconTabProvider && ( (IconTabProvider) pager.getAdapter() ).isIconMode() ) {
		...
		updateTabSelected( pager.getCurrentItem() );
		updateTabStyles();
	...
	private void updateTabSelected( int position ) {

		imageButtons.get( position ).setSelected( true );
		if ( null != selectedView )
			selectedView.setSelected( false );
		selectedView = imageButtons.get( position );
	}
	...
	private void addTab( final int position, View tab ) {
		...
		tab.setOnClickListener( new OnClickListener() {
			...
			public void onClick( View v ) {
				...
				if ( position == pager.getCurrentItem() && null != onScrollingToTopListener )
					onScrollingToTopListener.onScrollingToTop( position );
		...
		imageButtons.add( tab );
	}

	private void updateTabStyles() {

		int selectedPosition = pager.getCurrentItem();
	...
	public void setEnabled( boolean enabled ) {
		...
		tabsContainer.setEnabled( enabled );
		for ( int i = 0; i < tabsContainer.getChildCount(); i++ )
			tabsContainer.getChildAt( i ).setEnabled( enabled );
	}
	...
	public interface OnScrollingToTopListener {

		public void onScrollingToTop( int currentPagePosition );
